...
void timer_iniciarCronometro(std::vector<std::any>* arrayTimer) {    
    if(arrayTimer->size() < 5) {

        Uint32 tempoAnterior = SDL_GetTicks();
        arrayTimer->push_back(tempoAnterior);

        bool iniciouCronometro = true;
        arrayTimer->push_back(iniciouCronometro);   

    } else {
        bool iniciouCronometro_ = std::any_cast<bool>((*arrayTimer)[4]);
        if(!iniciouCronometro_) {
            Uint32 tempoAnterior = SDL_GetTicks();
            arrayTimer->push_back(tempoAnterior);
...
bool timer_verificar(std::vector<std::any>* arrayTimer) {
    ...
    } else if(tempoAtual - tempoAnterior >= intervalo && loop == true) {
        (*arrayTimer)[3] = SDL_GetTicks();
        return true;
    }
